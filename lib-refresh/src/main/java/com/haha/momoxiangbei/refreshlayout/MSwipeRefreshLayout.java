...
/**
 * 扩展的SwipeRefreshLayout，可以无缝切换google的SwipeRefreshLayout
 * <p>
...
public class MSwipeRefreshLayout extends ViewGroup {

    private static final String TAG = "MSwipeRefreshLayout";

    private static final int HEADER_VIEW_HEIGHT = 50;        // HeaderView高度(dp)
    private static final int FOOTER_VIEW_HEIGHT = 80;        // footerView高度(dp)

    private static final int ANIMATE_TO_TRIGGER_DURATION = 200;    // 触发跟随收拾滑动的动画时间
    private static final int ANIMATE_TO_START_DURATION = 200;    // 滑动到起始位置时的动画间时间
    ...
    private int mHeaderViewWidth;  // headerView宽度
    private int mFooterViewWidth;  // footerView宽度
    private View mTarget; // MSwipeRefreshLayout包含的View，比如RecyclerView,ListView,ScrollView...
    private OnRefreshListener mListener;// 下拉刷新listener
    ...
    private boolean mIsBeingDragged;    //是否正在拖动
    private int mActivePointerId = INVALID_POINTER;
    private boolean mScale;
    private boolean mReturningToStart;   // 返回到顶部
    ...
    // OffsetTop为内部view距离屏幕顶部的偏移量（内部View包含headerView）
    protected int mOriginalOffsetTop;        // 未刷新时，距离屏幕顶部的距离，值为负的headerView高度
    private int mCurrentTargetOffsetTop;     // 当前距离屏幕顶部的距离
    protected int mFrom;           // 等于mCurrentTargetOffsetTop的值
    private Animation mScaleAnimation;
    private Animation mScaleDownAnimation;
    private Animation mScaleDownToStartAnimation;
    // 滑动时最大的偏移量px （ 通过DEFAULT_CIRCLE_TARGET设置 ）
    private float mSpinnerFinalOffset;
    private boolean mNotify;
    private int mHeaderViewHeight;
    private int mFooterViewHeight;
    private boolean mUsingCustomStart;
    private boolean targetScrollWithLayout = true;   // 目标View跟随手指的滑动而滑动
    private int pushDistance = 0;
    private CircleProgressView defaultProgressView = null;
    private boolean usingDefaultHeader = true;    // 使用默认Header
    private float density = 1.0f;
    private boolean isProgressEnable = true;
    private static final String LOG_TAG = MSwipeRefreshLayout.class.getSimpleName();
    ...
    private void createHeaderViewContainer() {
        ...
        // 在headerView中添加defaultProgressView
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 测量mTarget、mHeadViewContainer、mFooterViewContainer
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        //  如果目标View不跟随手指的滑动而滑动，将下拉偏移量设置为0
...
//        int pointerIndex;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mActivePointerId == INVALID_POINTER) {
                    Log.d(TAG, "onInterceptTouchEvent: Got ACTION_MOVE event but don't have an active pointer id.");
                    return false;
                }
                ...
                if (isChildScrollToBottom()) {
                    ...
                    // 判断是否下拉的距离足够
                    if (yDiff > mTouchSlop && Math.abs(yDiff) > Math.abs(xDiff) && !mIsBeingDragged) {
                        // 正在下拉
                        mIsBeingDragged = true;
                    }
                }
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        // 将新添加的headerView和footerView放到最后绘制
    ...
    public boolean isChildScrollToBottom() {
        ...
        if (mTarget instanceof RecyclerView) {
            ...
            if (lastPos > 0 && count > 0 && lastPos == count - 1) {
                ...
                if (item.getBottom() <= getBottom()) { //重要修改！！！必须考虑SuperSwipeRefreshLayout本身的footer
                    return true;
                }
            }
            return false;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
                if (pointerIndex < 0) {
                    Log.d(TAG, "handlerPullTouchEvent: Got ACTION_MOVE event but have an invalid active pointer id.");
                    return false;
                }
            ...
            case MotionEvent.ACTION_CANCEL: {
                if (mActivePointerId == INVALID_POINTER) {
                    if (action == MotionEvent.ACTION_UP) {
                        Log.d(TAG, "handlerPullTouchEvent: Got ACTION_UP event but don't have an active pointer id.");
                    }
                    return false;
                }
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                Log.d(TAG, "handlerPushTouchEvent: debug:onTouchEvent ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
                if (pointerIndex < 0) {
                    Log.d(TAG, "handlerPushTouchEvent: Got ACTION_MOVE event but have an invalid active pointer id.");
                    return false;
                }
                final float y = MotionEventCompat.getY(ev, pointerIndex);
            ...
            case MotionEvent.ACTION_CANCEL: {
                if (mActivePointerId == INVALID_POINTER) {
                    if (action == MotionEvent.ACTION_UP) {
                        Log.d(TAG, "handlerPushTouchEvent: Got ACTION_UP event but don't have an active pointer id.");
                    }
                    return false;
                }
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
    ...
    /**
     * 重置Target、headerView、footerView位置
     *
     * @param delay 延迟重置时间
     */
    ...
    private class CircleProgressView extends View implements Runnable {
        private static final int PEROID = 16; //绘制周期16ms
     ...
     * Adapter
     */
    public class OnRefreshListenerAdapter extends OnRefreshListener {
    ...
    public class OnPushLoadMoreListenerAdapter extends OnPushLoadMoreListener {
     ...
     * 设置子View是否跟随手指的滑动而滑动
     *
     * @param targetScrollWithLayout 是否跟随手指滑动
     */
     ...
     * 设置上拉加载更多接口回调
     *
     * @param onPushLoadMoreListener OnPushLoadMoreListener
     */
    ...
    /**
     * 设置刷新状态，不要在使用手势刷新过程中使用
     *
     * @param refreshing 要设置的刷新状态
     */
     ...
     * @param enableLoadMore
     */
    public void setEableLoadMore(boolean enableLoadMore) {
        this.enableLoadMore = enableLoadMore;
    }
     ...
     * 上拉加载更多
     */
    public static abstract class OnPushLoadMoreListener {
